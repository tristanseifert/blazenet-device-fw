...
void Task::DispatchCommand(const uint8_t cmd, etl::span<uint8_t> payload) {
    Logger::Notice("%s cmd %02x(%u)", ">>", cmd, payload.size());

    if(!payload.empty()) {
        Logger::Notice("Payload: %02x %02x %02x %02x %02x %02x %02x %02x",
                gPayloadBuffer[0], gPayloadBuffer[1], gPayloadBuffer[2], gPayloadBuffer[3],
                gPayloadBuffer[4], gPayloadBuffer[5], gPayloadBuffer[6], gPayloadBuffer[7]);
    }
...
void Task::DispatchCommandWithResponse(const uint8_t cmd, const size_t numResponseBytes) {
    Ecode_t err;
    Logger::Notice("%s cmd %02x(%u)", "<<", cmd, numResponseBytes);

    // TODO; execute the command
    static const etl::array<const uint8_t, 256> kData{{
    ...
    // send response
    err = SPIDRV_STransmit(sl_spidrv_eusart_host_handle, kData.data(), numResponseBytes,
